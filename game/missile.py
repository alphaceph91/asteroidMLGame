...
# Load missile image (original image should be oriented upward, for example)
...
class Missile:
    def __init__(self, x, y, target=None, speed=10):
        ...
        self.y = y
        self.speed = speed
        self.target = target  # target asteroid (if heat-seeking)
        if self.target is not None:
            self.update_direction()
        else:
            self.vel_x = 0
            self.vel_y = -speed  # default upward movement
    ...
    def draw(self, screen):
        ...
        # Adjust the angle by subtracting 90 degrees so that an image originally
        # pointing upward rotates correctly.
    ...
    def get_rect(self):
        # Create a rect based on current x,y and missile size
